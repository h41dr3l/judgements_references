#change webscraper sso file to this 
import scrapy
...
with open("./data/legis_name.txt", "w") as f:
  f.write("Legislation names\n")
...
class SsoSpider(scrapy.Spider):
  ...
  def scrape_all(self, response):   
    ...
    # with open(f"./data/legis_name.txt", "a") as f:
    #   f.write(f"{len(acts)}\n")
    ...
    for act in acts:
      ...
      item["link"] = link
      with open(f"./data/legis_name.txt", "a") as f:
        f.write(f"{item['title']}, {item['shorthand']}\n")
      #finds subsid link - 
      if self.include_subsid:
